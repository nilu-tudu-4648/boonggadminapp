...
interface FuturePickupScreenProps {

}

const FuturePickupScreen: React.FC<FuturePickupScreenProps> = ({ navigation }: any) => {
  const { loading, futurepickUp, cancelrequestRentBookingData, transferRentbooking, webuserupdate,
  ...
  const [bookingforCheckIn, setBookingforCheckIn] = useState<any>({})
  // ref
  ...
  const [futurepickUplocal, setfuturepickUplocal] = React.useState<any>(futurepickUp);
  ...
  const onChangeSearch = useCallback((query: string) => {
    if (query) {
      setfuturepickUplocal(futurepickUp.filter(ite =>
      ...
      setfuturepickUplocal(futurepickUp)
    }
  ...
  useEffect(() => {
    if (activeRouteName === 'FuturePickup') {
      apiCalls()
    }
          ...
          >
            <View style={styles.contentContainer}>
              <TouchableOpacity onPress={() => closeSheet()}>
                <AppText style={[tw`my-1`, { color: colors.purple, fontSize: 14 }]}>Future Pickup</AppText>
              </TouchableOpacity>
...
export default React.memo(FuturePickupScreen);