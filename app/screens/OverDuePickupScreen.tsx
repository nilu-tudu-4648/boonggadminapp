...
import {
  ...
  getrentbookinguserOngoing, getUserbyMobileNo as getuser, getrentpoolList
} from '@/store/pickUpStore';
import { createBookingPayDataReceived, getUserbyMobileNo, stepCountValuesaveToReducers, getrentpoolList as getrent } from "@/store/createBooking";
...
interface OverDuePickupScreenProps {

}

const OverDuePickupScreen: React.FC<OverDuePickupScreenProps> = ({ navigation }: any) => {

  const { loading, overduepickUp, cancelrequestRentBookingData, transferRentbooking, webuserupdate, fetchBikestransferRentbooking }
    = useAppSelector((state: any) => state.entities.pickupStore);
  const { storeDetail, currentCity } = useAppSelector((state: RootState) => state.entities.searchBikes);
  ...
  const [overduepickUplocal, setoverduepickUplocal] = React.useState<any>(overduepickUp);
  ...
  const toggleDialog = useCallback((str: any) => {
    if (str) return setvisibleDialog(str)
    setvisibleDialog('')
  }, [])
  const openImageDialog = (urls: object) => {
    setimageUrl(urls)
    toggleDialog("imagesdialog")
  }
  const onRefresh = () => {
    setrefreshing(true)
    apiCalls()
  ...
  const onChangeSearch = useCallback((query: string) => {
    if (query) {
      setoverduepickUplocal(overduepickUp.filter(ite =>
      ...
      setoverduepickUplocal(overduepickUp)
    }
    setSearchQuery(query)
  }, [searchQuery])
  const checkInFunc = useCallback((item: any) => {
  ...
  useEffect(() => {
    if (activeRouteName === 'OverDuePickup') {
      apiCalls()
    }
  ...
  return (
    <View style={[tw`bg-white`, { flex: 1 }]}>
      ...
      <TransferBookingDialog
        ...
        fetchBikestransferRentbooking={fetchBikestransferRentbooking}
      ...
      />
      <Portal>
        <Dialog style={tw`p-2 bg-white`} visible={visibleDialog === "cancel"} onDismiss={() => toggleDialog('')}>
          <AppText style={[tw`m-3 font-bold `, { color: colors.purple, fontSize: 16 }]}>{`Cancel Booking`}</AppText>
          ...
          <StyleButton title="Cancel It" borderColor="#17A1FA" textStyle={{ fontSize: 11 }} style={{ width: '32%', margin: 10, height: 32, alignSelf: 'flex-end' }} onPress={() => {
            cancelButtonCalled()
          }} />
        </Dialog>
      </Portal>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <CheckInDetailsCard
          ...
          setvisibleDialog={toggleDialog as () => void}
          city={_.get(currentCity, '_city.name')}
        />}
        onRefresh={onRefresh}
          ...
          >
            <View style={styles.contentContainer}>
              <TouchableOpacity onPress={() => closeSheet()}>
                <AppText style={[tw`my-1`, { color: colors.purple, fontSize: 14 }]}>{`OverDue Pickup`}</AppText>
              </TouchableOpacity>
              ...
              {/* timebox */}
              <View style={[tw`flex-row justify-between my-1.5`, { height: 60 }]}>
                <View style={tw`flex-row justify-between p-1 bg-gray-200 rounded-md w-4.7/10 `}>
                  ...
                  {/* <View>
                    <AppText style={tw`text-gray-200`}>{"f"}</AppText>
                    <AppText style={[tw``, { fontSize: 9 }]}>8:00 PM</AppText>
                  </View> */}
                </View>
                <View style={tw`flex-row justify-between p-1 bg-gray-200 rounded-md w-4.7/10 `}>
                  <View>
              ...
              {/* kycbox */}
              <View style={[tw`bg-gray-100 `, { height: 130 }]}>
                <AppText style={[tw`ml-2`, { fontSize: 9 }]}>KYC</AppText>
                <View style={[tw`flex-row justify-between`, {}]}>
                  ...
                  <View style={tw`w-1/2 items-center`}>
                    {
                      _.get(bookingforCheckIn, "_webuserId.profile.newDriving") ?
                        <TouchableOpacity style={tw`w-full items-center h-7/10`} onPress={() => {
                          openImageDialog(bookingforCheckIn)
                        }}>
                          <Image source={{ uri: _.get(bookingforCheckIn, "_webuserId.profile.newDriving") }} style={[tw`w-9/10 rounded-md`, { height: '100%' }]} />
                        </TouchableOpacity>
                        : <View style={[tw`w-9/10 bg-gray-300 rounded-lg`, { height: '68%' }]} />
                    }
              ...
              <View style={[tw``, { height: 60, bottom: 5 }]}>
                <AppButton
                  onPress={() => {
...
export default OverDuePickupScreen;